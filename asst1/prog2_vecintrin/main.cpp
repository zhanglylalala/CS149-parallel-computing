...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  __cs149_vec_float zero_f = _cs149_vset_float(0.f), inf = _cs149_vset_float(9.999999f);
  __cs149_vec_float one_f = _cs149_vset_float(1.f);
  __cs149_vec_int y, count, index;
  __cs149_vec_int zero = _cs149_vset_int(0), one = _cs149_vset_int(1), num = _cs149_vset_int(N);
  __cs149_mask maskAll, maskIsYZero, maskIsYNotZero, maskIsCountZero, maskIsResultINF, maskOutofBound;
  ...
  for (int i = 0; i < N; i += VECTOR_WIDTH)
  {
    ...
    _cs149_vmove_float(result, one_f, maskIsYZero);                 //    output[i] = 1.f;

    maskIsYNotZero = _cs149_mask_not(maskIsYZero);                  // } else {
    
    _cs149_vmove_float(result, x, maskIsYNotZero);                  //    result = x;
    _cs149_vsub_int(count, y, one, maskIsYNotZero);                 //    count = y - 1;

    _cs149_vgt_int(maskIsCountZero, count, zero, maskAll);          //    while (count > 0) {
    while (_cs149_cntbits(maskIsCountZero))
    {
      _cs149_vmult_float(result, result, x, maskIsCountZero);       //    result *= x;
      _cs149_vsub_int(count, count, one, maskIsCountZero);          //    count--; }
      _cs149_vgt_int(maskIsCountZero, count, zero, maskAll);
    }
    
    _cs149_vgt_float(maskIsResultINF, result, inf, maskAll);        //    if(result > 9.999999f) {
    _cs149_vmove_float(result, inf, maskIsResultINF);               //      result = 9.999999f;}

    if (i + VECTOR_WIDTH > N)
    {
      maskOutofBound = _cs149_init_ones(N % VECTOR_WIDTH);
    }
    else
    {
      maskOutofBound = _cs149_init_ones();
    }
    _cs149_vstore_float(output + i, result, maskOutofBound);
  }
...
float arraySumVector(float* values, int N) {
  ...
  //
  __cs149_vec_float value, partial_sum;
  __cs149_mask maskAll, maskFirst;

  partial_sum = _cs149_vset_float(0.f);

  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    maskAll = _cs149_init_ones();
    _cs149_vload_float(value, values + i, maskAll);
    _cs149_vadd_float(partial_sum, partial_sum, value, maskAll);
  }
  ...
  float final_sum;
  maskFirst = _cs149_init_ones(1);
  _cs149_vstore_float(&final_sum, partial_sum, maskFirst);
